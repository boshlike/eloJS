...
// when it is minus 1 it never approaches 0 or 1 so is undefined. Need to invert the number
// // Your code here.
...
// only one character (for example, "b"). The first character has position 0, which causes the last one to be found at position string.length - 1. In other words, a two-character string has length 2, and its characters have positions 0 and 1.
...
function countBs1(str) {
    let count = 0; // Set an empty variable for the count
    for (let i = 0, len = str.length; i < len; i++) {
          if (str[i] === 'B') count++;
    }
    return count; // Return the count, exit the function
}

console.log('countBs1(\'BBC\'): ', countBs1('BBC'));
// → 2

function countChar(str, char) {
    let count = 0; // Set an empty variable for the count
    for (let i = 0, len = str.length; i < len; i++) {
          if (str[i] === char) count++; // Replace the 'B' with char input for function
    }
    return count; // Return the count, exit the function
}
...
// Rewrite countBs to make use of this.
function countBs2(str) {
    return countChar(str, 'B'); // Callback function that returns the count with Bs
}
console.log('countBs2(\'BBC\'): ', countBs2('BBC'));
console.log('\n');
